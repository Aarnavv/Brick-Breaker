// Declaring the variables required to generate the brick structures
var xCor = 90;
var yCor = 100;
var timesToRun = 23;
...
function setup() {
  ...
  // Creating the ball and the paddle
...
function draw() {
  ...
  ball.bounceOff(edges[2]) // Making the ball bounce-off the left edge (edges[2] == top edge)
  ball.bounceOff(edges[0]) // Making the ball bounce-off the left edge (edges[0] == left edge)
  ball.bounceOff(edges[1]) // Making the ball bounce-off the left edge (edges[1] == right edge)
  ...
  // Deciding game win or game lose conditions
  if (gameState == "play") {
...
// Deleting the bricks when the ball touches them
...
function drawBricks() {
  ...
  // for loop to generate the brick structure
  for (var i = 1; i <= 12; i++) {
